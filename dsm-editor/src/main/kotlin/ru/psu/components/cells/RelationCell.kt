...
class RelationCell(private val controller: MainController): ListCell<MLRelation>() {
    init {
        try {
            val loader = FXMLLoader(javaClass.getResource("RelationCell.fxml"))
            loader.setRoot(this)
            loader.load<Any>()
        } catch (e: IOException) {
    ...
    override fun updateItem(item: MLRelation?, empty: Boolean) {
