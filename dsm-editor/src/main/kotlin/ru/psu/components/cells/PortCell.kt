...
class PortCell(private val controller: MainController): ListCell<MLPort>() {
    init {
        try {
            val loader = FXMLLoader(javaClass.getResource("PortCell.fxml"))
            loader.setRoot(this)
            loader.load<Any>()
        } catch (e: IOException) {
    ...
    override fun updateItem(item: MLPort?, empty: Boolean) {
